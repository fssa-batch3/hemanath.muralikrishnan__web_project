const created_products = JSON.parse(localStorage.getItem("product_list"));

const sorty_json = JSON.parse(localStorage.getItem("product_list"));
...
const product_english_name = document.getElementById(
  "edit-product-english-name"
);
...
function list_products(array = []) {
  ...
  array.forEach((item, index) => {
    ...
    const td_pro_id = document.createElement("td");
    ...
    if (item.status) {
      td_pro_avail_or_not.setAttribute(
        "onclick",
...
const filter_by_cat = document.getElementById("select-category-filter");
...
filter_by_cat.addEventListener("change", () => {
  newarray = [];

  const selectedValue = filter_by_cat.value;

  if (selectedValue !== "00") {
    created_products.filter((val) => {
...
select_status.addEventListener("change", () => {
  ...
  const selectedValue = select_status.value;

  const filter_cat_value = filter_by_cat.value;

  if (selectedValue === "yes") {
    created_products.filter((status_obj) => {
      if (
        status_obj.category.id === filter_cat_value &&
...
const search_by_pro_name = document.getElementById("site-search");
...
search_by_pro_name.addEventListener("keyup", function () {
  ...
  const get_cat_value = filter_by_cat.value;

  if (get_cat_value !== "00") {
    created_products.filter((search_fil) => {
      const pro_name = search_fil.name.eng;

      const lc_pro_name = pro_name.toLowerCase();

      if (
        search_fil.category.id === get_cat_value &&
        lc_pro_name.includes(search)
      ) {
...
const sortyby = document.getElementById("sorty-by");

sortyby.addEventListener("change", () => {
  let sortby;

  const sortybyvalue = sortyby.value;

  if (sortybyvalue === "default") {
    list_products(created_products);
  } else if (sortybyvalue === "nameatoz") {
    sortby = sorty_json.sort((a, b) => {
      const x = a.name.eng.toLowerCase();

      const y = b.name.eng.toLowerCase();
    ...
    sortby = sorty_json.sort((a, b) => {
      const x = a.avail_stock.num;

      const y = b.avail_stock.num;
...
function editproduct(id) {
  created_products.find((obj) => {
    if (id === obj.id) {
      ...
      const copy_product = obj;

      localStorage.setItem("copy", JSON.stringify(copy_product));

      const edit_product = JSON.parse(localStorage.getItem("copy"));
...
function displaypricelist() {
  ...
  const list_type = JSON.parse(localStorage.getItem("copy"));
...
function deletepricelist(index) {
  const localitems = JSON.parse(localStorage.getItem("copy"));
...
edit_form.addEventListener("submit", (e) => {
  ...
  const available_stock_input_unit = document.querySelector(
