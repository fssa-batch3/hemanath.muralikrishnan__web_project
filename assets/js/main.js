const created_products = [
  {
    id: 0,
    name: {
      eng: "Apple Green",
      tam: "பச்சை ஆப்பிள்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/apple-green.jpeg",
      alt: "Apple Green",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "Green apples are a boon to those who wish to shed the extra fat in their body. This fruit has low calories and rich in fiber that helps you to fight the hunger pangs when you have it early morning in an empty stomach. It is low in sodium, sugar, and fat that helps it to boost the process of calorie burning to achieve your weight loss goals much faster.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "25",
        unit: "gm",
      },
      kcal: "95",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹200",
        unit: "kg",
        qty: "1.2",
        rs: "200",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 1,
    name: {
      eng: "Avocado",
      tam: "அவோகேடோ",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/avacado.png",
      alt: "Avocado",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "Avocados are a source of key nutrients, including healthy fats and fiber. They also contain anti-inflammatory and antioxidant compounds and may help reduce heart disease risk.",
    nutritions: {
      protein: {
        num: "4",
        unit: "gm",
      },
      carbo: {
        num: "17",
        unit: "gm",
      },
      kcal: "322",
    },
    avail_stock: {
      num: "25",
      unit: "kg",
      into_gram: "25000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.2kg - ₹220",
        unit: "kg",
        qty: "1.2",
        rs: "220",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 2,
    name: {
      eng: "Blueberry",
      tam: "கிவி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/blueberry.jpeg",
      alt: "Blueberry",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "Blueberries contain vitamins, minerals, and antioxidants that provide notable health benefits. For example, blueberries are rich in vitamin K, which plays an important role in promoting heart health. The vitamin is also important to bone health and blood clotting.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "11",
        unit: "gm",
      },
      kcal: "42",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹120",
        unit: "gm",
        qty: "500",
        rs: "120",
      },
      {
        text: "1.5kg - ₹200",
        unit: "kg",
        qty: "1.5",
        rs: "200",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 3,
    name: {
      eng: "Dragon Fruit",
      tam: "டிராகன் பழம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/dragon-fruit.png",
      alt: "Dragon Fruit",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "It can boost your iron levels. Iron is important for moving oxygen through your body and giving you energy, and dragon fruit has iron. And the vitamin C in dragon fruit helps your body take in and use the iron.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "22",
        unit: "gm",
      },
      kcal: "102",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.5kg - ₹170",
        unit: "kg",
        qty: "1.5",
        rs: "170",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 4,
    name: {
      eng: "Kiwi Green",
      tam: "கிவி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/kiwi-green.png",
      alt: "Kiwi Green",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "The vitamins, fiber, and antioxidants can provide essential health benefits. The flesh is rich with vitamins that stimulate immunity and reduce the risk of disease.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "11",
        unit: "gm",
      },
      kcal: "46",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.2kg - ₹150",
        unit: "kg",
        qty: "1.2",
        rs: "150",
        into_gram: "1200",
      },
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 5,
    name: {
      eng: "Pears Green",
      tam: "பச்சை பேரிக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/pears-green.png",
      alt: "Pears Green",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "Pears are good for your digestion, heart and blood glucose levels. They're also free radical warriors. Eat pears raw or try cooking with them in recipes like Baked Oatmeal with Pears, Roasted Butternut Squash & Pear Quinoa Salad and Pear Custard Pie.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "27",
        unit: "gm",
      },
      kcal: "101",
    },
    avail_stock: {
      num: "25",
      unit: "kg",
      into_gram: "25000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.5kg - ₹220",
        unit: "kg",
        qty: "1.5",
        rs: "220",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 6,
    name: {
      eng: "Strawberry",
      tam: "ஸ்ட்ராபெர்ரி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/strawbeery.jpeg",
      alt: "Strawberry",
    },
    category: {
      id: "01",
      name: "Exotic Fruits",
    },
    description:
      "Strawberries are a favorite summer fruit. The sweet berries appear in everything from yogurt to desserts and even salads. Strawberries, like most berries, are a low-glycemic fruit, making them a tasty option for people looking to control or reduce their glucose levels.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "23",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.5kg - ₹250",
        unit: "kg",
        qty: "1.5",
        rs: "250",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 7,
    name: {
      eng: "Broccoli",
      tam: "பச்சை பூக்கோசு",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/broccoli.png",
      alt: "Broccoli",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "One cup of broccoli provides about 2 grams of fiber, which supports digestive health, protects your heart by lowering cholesterol levels, and keeps you feeling full.34 And regularly consuming enough fiber helps support blood sugar and insulin regulation for steady, even energy.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "15",
    },
    avail_stock: {
      num: "25",
      unit: "kg",
      into_gram: "25000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.5kg - ₹200",
        unit: "kg",
        qty: "1.5",
        rs: "200",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 8,
    name: {
      eng: "Butter Beans",
      tam: "ப. பீன்ஸ்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/butter-beans.png",
      alt: "Butter Beans",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Diabetics have to be careful about the kind of food items that they include in their diet. Butter beans happen to be one of the ideal choices for a diabetic meal plan due to their high dietary fiber content. The high dietary fiber content helps in regulating the blood sugar levels in the body.",
    nutritions: {
      protein: {
        num: "5",
        unit: "gm",
      },
      carbo: {
        num: "17",
        unit: "gm",
      },
      kcal: "466",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.2kg - ₹100",
        unit: "kg",
        qty: "1.2",
        rs: "100",
        into_gram: "1200",
      },
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 9,
    name: {
      eng: "Capsicum Red",
      tam: "சிவப்பு குடை மிளகாய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/capsicum-red.png",
      alt: "Capsicum Red",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Red capsicums, for example, contain more phytonutrients than any of the other capsicums, making them the type with the highest antioxidant content. It has 11 times more beta-carotene, and one and a half times more vitamin C than green varieties.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "7",
        unit: "gm",
      },
      kcal: "30",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "500gm - ₹75",
        unit: "gm",
        qty: "500",
        rs: "75",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 10,
    name: {
      eng: "Celery",
      tam: "செலரி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/celery.png",
      alt: "Celery",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Widely recognized as one of the healthiest snacks available, celery is a wonderful source of fiber. Its fiber content is especially impressive given the vegetable's low calorie count. This makes it a great option for those concerned with losing weight or maintaining healthy digestion.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "1",
        unit: "gm",
      },
      kcal: "80",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.2kg - ₹200",
        unit: "kg",
        qty: "1.2",
        rs: "200",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 11,
    name: {
      eng: "Chinese Cabbage",
      tam: "சீன முட்டைக்கோஸ்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/chinesecabbage.png",
      alt: "Chinese Cabbage",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "New bone materials alongside Vitamin K and Calcium in Chinese cabbage play an important role of keeping the teeth and bones strong. Calcium also helps enhances nerve function while Vitamin K promotes fat breakdown and blood clotting. A cup of bok choy contains 64% Vitamin K and 16% Calcium.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "3",
        unit: "gm",
      },
      kcal: "12",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 12,
    name: {
      eng: "Country Cucumber",
      tam: "நாட்டு வெள்ளரிக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/countrycucumber.png",
      alt: "Country Cucumber",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "The vitamin K helps blood clot and keep your bones healthy. Vitamin A has many jobs, like helping with vision, the immune system, and reproduction. It also makes sure organs like your heart, lungs, and kidneys work the way they should.",
    nutritions: {
      protein: {
        num: "3",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "30",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
      {
        text: "1.2kg - ₹140",
        unit: "kg",
        qty: "1.2",
        rs: "140",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 13,
    name: {
      eng: "Lettuce Green",
      tam: "பச்சை லீட்டஸ் கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/greenlettuce.png",
      alt: "Lettuce Green",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Water makes up over 95% of raw lettuce. As a result, eating lettuce hydrates the body. Although drinking liquids is necessary, water in foods can also significantly contribute to hydration.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "23",
        unit: "gm",
      },
      kcal: "20",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹120",
        unit: "gm",
        qty: "500",
        rs: "120",
      },
      {
        text: "1.2kg - ₹170",
        unit: "kg",
        qty: "1.2",
        rs: "170",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 14,
    name: {
      eng: "Leeks",
      tam: "லீக்ஸ்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/leeks.png",
      alt: "Leeks",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Leek requires well-drained, fertile soil to flourish. In general, it is cultivated as an annual crop in many parts of Europe and Asia. Farmers either sow seeds or transplant already raised seedlings sets in the field that take about 100-120 days to harvest.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "15",
        unit: "gm",
      },
      kcal: "61",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹160",
        unit: "kg",
        qty: "1.2",
        rs: "160",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 15,
    name: {
      eng: "Lettuce Iceberg",
      tam: "ஐஸ்பெர்க் லீட்டஸ் கீ",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/lettuce-iceberg.jpeg",
      alt: "Lettuce Iceberg",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Iceberg lettuce is a great bridge food for people who don’t eat enough other vegetables. It has a mild, sweet flavor and a pleasant crunch. And while iceberg lettuce gets a bad reputation for not being as nutritious as other lettuces, it’s packed with Vitamin A, Vitamin K, and folate. Iceberg lettuce can offer a range of important health benefits for you and your family.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "2",
        unit: "gm",
      },
      kcal: "10",
    },
    avail_stock: {
      num: "25",
      unit: "kg",
      into_gram: "25000",
    },
    quantity: [
      {
        text: "500gm - ₹75",
        unit: "gm",
        qty: "500",
        rs: "75",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 16,
    name: {
      eng: "Red Cabbage",
      tam: "சிவப்பு கோஸ்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/redcabbage.png",
      alt: "Red Cabbage",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Like other cruciferous vegetables, red cabbage contains various nutrients that your body needs. Research also suggests that diets high in cruciferous vegetables like red cabbage may help protect against some types of cancer.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "7",
        unit: "gm",
      },
      kcal: "28",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹95",
        unit: "gm",
        qty: "500",
        rs: "95",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 17,
    name: {
      eng: "Lettuce Red",
      tam: "சிவப்பு லீட்டஸ் கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/redlettuce.png",
      alt: "Lettuce Red",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Water makes up over 95% of raw lettuce. As a result, eating lettuce hydrates the body. Although drinking liquids is necessary, water in foods can also significantly contribute to hydration.",
    nutritions: {
      protein: {
        num: "5",
        unit: "gm",
      },
      carbo: {
        num: "16",
        unit: "gm",
      },
      kcal: "75",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.0kg - ₹250",
        unit: "kg",
        qty: "1.0",
        rs: "250",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 18,
    name: {
      eng: "Turnip",
      tam: "டர்னிப்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/turnip.png",
      alt: "Turnip",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "One medium turnip has 233 mg of potassium. Your body needs that mineral for just about everything, including helping your nerves send signals and keeping your muscles and heart in good working shape. When you don't get enough, you're at risk for high blood pressure, stroke, kidney stones, brittle bones, and high blood sugar.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "8",
        unit: "gm",
      },
      kcal: "34",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹75",
        unit: "gm",
        qty: "500",
        rs: "75",
      },
      {
        text: "1.5kg - ₹150",
        unit: "kg",
        qty: "1.5",
        rs: "150",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 19,
    name: {
      eng: "Capsicum Yellow",
      tam: "மஞ்சள் குடைமிளகாய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/yellowcapsicum.png",
      alt: "Capsicum Yellow",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Yellow Bell Pepper is a variety of the colored Capsicums. It gets it bright pigment from the pH of the soil and is basically a ripened green pepper. Yellow Peppers are very mild in flavour and sweet in taste. They are quite juicy and can be cooked very easily.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "12",
        unit: "gm",
      },
      kcal: "50",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "250gm - ₹40",
        unit: "gm",
        qty: "250",
        rs: "40",
      },
      {
        text: "500gm - ₹80",
        unit: "gm",
        qty: "500",
        rs: "80",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 20,
    name: {
      eng: "Zucchini Green",
      tam: "சீமை சுரைக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/Zucchini-Green.png",
      alt: "Zucchini Green",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Zucchini is packed with many important vitamins, minerals, and antioxidants. It has a high fiber content and a low calorie count. Fiber plays an important role in digestion and may limit the likelihood of suffering from a variety of GI issues.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "33",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹90",
        unit: "gm",
        qty: "500",
        rs: "90",
      },
      {
        text: "1.5kg - ₹150",
        unit: "kg",
        qty: "1.5",
        rs: "150",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 21,
    name: {
      eng: "Zucchini yellow",
      tam: "சீமை சுரைக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/Zucchini-yellow.jpeg",
      alt: "Zucchini yellow",
    },
    category: {
      id: "02",
      name: "Exotic Veggies",
    },
    description:
      "Zucchini is packed with many important vitamins, minerals, and antioxidants. It has a high fiber content and a low calorie count. Fiber plays an important role in digestion and may limit the likelihood of suffering from a variety of GI issues.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "33",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "250gm - ₹50",
        unit: "gm",
        qty: "250",
        rs: "50",
      },
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 23,
    name: {
      eng: "Apple Royal Gala",
      tam: "ஆப்பிள் ராயல் காலா",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/apple-royal-gala.jpeg",
      alt: "Apple Royal Gala",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "This mighty fruit is known to maintain oral health. Thanks to malic acid that keeps your teeth white and shiny. It is widely used by companies to prepare commercial whitening toothpaste. It is worth mentioning that chewing apple stimulates saliva production that keeps risks of tooth decay at bay.",
    nutritions: {
      protein: {
        num: "26",
        unit: "gm",
      },
      carbo: {
        num: "81",
        unit: "gm",
      },
      kcal: "72",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.0kg - ₹220",
        unit: "kg",
        qty: "1.0",
        rs: "220",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 24,
    name: {
      eng: "Apple Shimla Big",
      tam: "சிம்லா ஆப்பிள்",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/apple-shimala-medium.png",
      alt: "Apple Shimla Big",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "This mighty fruit is known to maintain oral health. Thanks to malic acid that keeps your teeth white and shiny. It is widely used by companies to prepare commercial whitening toothpaste. It is worth mentioning that chewing apple stimulates saliva production that keeps risks of tooth decay at bay.",
    nutritions: {
      protein: {
        num: "26",
        unit: "gm",
      },
      carbo: {
        num: "81",
        unit: "gm",
      },
      kcal: "72",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹150",
        unit: "gm",
        qty: "500",
        rs: "150",
      },
      {
        text: "1.2kg - ₹250",
        unit: "kg",
        qty: "1.2",
        rs: "250",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 25,
    name: {
      eng: "Apple Shimla Medium",
      tam: "சிம்லா ஆப்பிள்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/apple-shimala.png",
      alt: "Apple Shimla Medium",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "This mighty fruit is known to maintain oral health. Thanks to malic acid that keeps your teeth white and shiny. It is widely used by companies to prepare commercial whitening toothpaste. It is worth mentioning that chewing apple stimulates saliva production that keeps risks of tooth decay at bay.",
    nutritions: {
      protein: {
        num: "26",
        unit: "gm",
      },
      carbo: {
        num: "81",
        unit: "gm",
      },
      kcal: "72",
    },
    avail_stock: {
      num: "25",
      unit: "kg",
      into_gram: "25000",
    },
    quantity: [
      {
        text: "500gm - ₹120",
        unit: "gm",
        qty: "500",
        rs: "120",
      },
      {
        text: "1.5kg - ₹250",
        unit: "kg",
        qty: "1.5",
        rs: "250",
        into_gram: "1500",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 26,
    name: {
      eng: "Baby Orange",
      tam: "மாண்டரின் ஆரஞ்சு",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/baby_orange.png",
      alt: "Baby Orange",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Besides strong immunity, vitamin C contains in baby orange is also good to help decrease the risk of growing cancer in the colon that can happen because of damaged DNA due to free radicals. The content of beta cryptoxhantin and carotenoids inside this fruit can also help to decrease the risk of lung cancer from developing.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "17",
        unit: "gm",
      },
      kcal: "72",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹120",
        unit: "gm",
        qty: "500",
        rs: "120",
      },
      {
        text: "250gm - ₹70",
        unit: "gm",
        qty: "250",
        rs: "70",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 27,
    name: {
      eng: "Banana Hill",
      tam: "மலை வாழைப்பழம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/banna-hill.jpeg",
      alt: "Banana Hill",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "According to USDA, bananas are a possibly rich source of protein, potassium, carbohydrates, and dietary fiber. They have zero fat and cholesterol, as well as negligible sodium.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "23",
        unit: "gm",
      },
      kcal: "89",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹60",
        unit: "gm",
        qty: "250",
        rs: "60",
      },
      {
        text: "500gm - ₹120",
        unit: "gm",
        qty: "500",
        rs: "120",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 28,
    name: {
      eng: "Coconut Flower",
      tam: "தென்னம்பூ",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/cocunt_flower.png",
      alt: "Coconut Flower",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Coconut flower nectar is a sweetener that has no adverse impact on the environment, has a maximum nutritional content, minimizes the effects of glycemic and fructose content for the body. All these are 1 benefits package that you can get from coconut flower nectar.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "8",
        unit: "gm",
      },
      kcal: "60",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹50",
        unit: "gm",
        qty: "500",
        rs: "50",
      },
      {
        text: "250gm - ₹30",
        unit: "gm",
        qty: "250",
        rs: "30",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 29,
    name: {
      eng: "Custard Apple",
      tam: "சீத்தாப்பழம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/custard-apple.png",
      alt: "Custard Apple",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Custard apples have numerous health benefits. The custard apple comes from the custard tree, whose leaves, bark, roots, and unripe fruit have medicinal properties. Despite the many health benefits of the custard tree, though, its seeds have a kernel (inner soft part) that can be toxic if consumed. Still, swallowing the seeds whole may not cause you any harm.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "24",
        unit: "gm",
      },
      kcal: "94",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹80",
        unit: "gm",
        qty: "500",
        rs: "80",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 30,
    name: {
      eng: "Grapes Paneer",
      tam: "பன்னீர் திராட்சை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/grapess-panner.jpeg",
      alt: "Grapes Paneer",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Grapes are full of vitamins, minerals and antioxidants. They’re also full of water, which can help keep you hydrated. Here’s how eating grapes can benefit your health.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "27",
        unit: "gm",
      },
      kcal: "104",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
      {
        text: "1.2kg - ₹140",
        unit: "kg",
        qty: "1.2",
        rs: "140",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 31,
    name: {
      eng: "Guava",
      tam: "கொய்யா பழம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/guva.png",
      alt: "Guava",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Guava is loaded with nutrients. Not only does it have more Vitamin C than oranges, guava is also rich in other antioxidants, and has been shown to have a number of great health benefits. Here are just a few of the benefits of eating this tropical fruit.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "8",
        unit: "gm",
      },
      kcal: "37",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹150",
        unit: "kg",
        qty: "1.2",
        rs: "150",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 32,
    name: {
      eng: "Mango Totapuri Raw",
      tam: "கிளிமூக்கு மாங்காய்",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/Mango-Totapuri-Raw.png",
      alt: "Mango Totapuri Raw",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "The vitamins, minerals, and antioxidants in mangos can provide important health benefits. For example, vitamin K helps your blood clot effectively and helps prevent anemia. It also plays an important role in helping strengthen your bones.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "14",
        unit: "gm",
      },
      kcal: "54",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 33,
    name: {
      eng: "Orange Australia",
      tam: "ஆஸ்திரேலியா ஆரஞ்சு",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/orange-austrialia.jpeg",
      alt: "Orange Australia",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Protects your cells from damage. Helps your body make collagen, a protein that heals wounds and gives you smoother skin. Makes it easier to absorb iron to fight anemia.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "16",
        unit: "gm",
      },
      kcal: "60",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹150",
        unit: "kg",
        qty: "1.2",
        rs: "150",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 34,
    name: {
      eng: "Pomegranate Medium",
      tam: "மீடியம் மாதுளை",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/Pomegranate-Medium.png",
      alt: "Pomegranate Medium",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Pomegranates can have up to three times more antioxidants than green tea or red wine. Antioxidants protect cells from damage, prevent diseases — such as cancer — and reduce inflammation and the effects of aging.",
    nutritions: {
      protein: {
        num: "5",
        unit: "gm",
      },
      carbo: {
        num: "53",
        unit: "gm",
      },
      kcal: "234",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
      {
        text: "1.2kg - ₹200",
        unit: "kg",
        qty: "1.2",
        rs: "200",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 35,
    name: {
      eng: "Sapota Paal",
      tam: "பால் சப்போட்டா",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/sapota.jpeg",
      alt: "Sapota Paal",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "The magnesium in sapota keeps the blood vessels up and running, and the potassium regulates blood pressure and circulation. It's also good to treat anaemia with, as it's also rich in iron.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "20",
        unit: "gm",
      },
      kcal: "83",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "500gm - ₹75",
        unit: "gm",
        qty: "500",
        rs: "75",
      },
      {
        text: "1.2kg - ₹130",
        unit: "kg",
        qty: "1.2",
        rs: "130",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 36,
    name: {
      eng: "Tender Coconut",
      tam: "இளநீர்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/tender-cocunt.jpeg",
      alt: "Tender Coconut",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Tender coconut is a natural rehydrating drink that has no added chemicals or harmful ingredients. It is the best drink to choose any day. It is very effective in keeping you hydrated, especially during summers.",
    nutritions: {
      protein: {
        num: "3",
        unit: "gm",
      },
      carbo: {
        num: "15",
        unit: "gm",
      },
      kcal: "354",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.0kg - ₹30",
        unit: "kg",
        qty: "1.0",
        rs: "30",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 37,
    name: {
      eng: "Watermelon Stripes",
      tam: "வரி தர்பூசணி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/watermeleon.png",
      alt: "Watermelon Stripes",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "Just one medium slice of watermelon gives you contains 9-11% of the vitamin A you need each day. This nutrient is one of the keys to keeping your eyes healthy. Foods are the best ways to get all the vitamins and minerals that your body needs.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "12",
        unit: "gm",
      },
      kcal: "46",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.0kg - ₹60",
        unit: "kg",
        qty: "1.0",
        rs: "60",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 38,
    name: {
      eng: "Bajji Chilli",
      tam: "பஜ்ஜி மிளகாய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/baji-chilli.jpeg",
      alt: "Bajji Chilli",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "The Baji Chilli is chilli of a long sized family of capsicum. While they are most widely used for frying and salads, they can also be used to be preserved and frozen. Some of you can't consume the food while the other half is almost in ecstasy from this vitamin C derivative. The reality is, for many of you, uncooked is more appealing and that too the chilli fried with flour-coated baji is exotic Indian street food.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "1",
        unit: "gm",
      },
      kcal: "10",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹70",
        unit: "gm",
        qty: "250",
        rs: "70",
      },
      {
        text: "500gm - ₹100",
        unit: "gm",
        qty: "500",
        rs: "100",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 39,
    name: {
      eng: "Bottle Gourd",
      tam: "சுரைக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/battle_guard.jpeg",
      alt: "Bottle Gourd",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "The low calorific value of bottle gourd makes it very beneficial for individuals who are on a weight loss regime and would like to get rid of some extra pounds. One can enjoy bottle gourd without the guilt of excessive calorie load as 100 grams of bottle gourd only provides 14 calories of energy.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "4",
        unit: "gm",
      },
      kcal: "14",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹50",
        unit: "gm",
        qty: "250",
        rs: "50",
      },
      {
        text: "500gm - ₹80",
        unit: "gm",
        qty: "500",
        rs: "80",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 40,
    name: {
      eng: "Big Onion",
      tam: "பெரிய வெங்காயம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/big-onion.png",
      alt: "Big Onion",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Many kinds of onions contain a wealth of chemicals that help fight cancer. Onions are among the richest food sources of a nutrient called quercetin, which is known to prohibit the activity or creation of cancer-causing elements. A quercetin-rich diet has been associated with a lower risk of developing lung cancer.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "7",
        unit: "gm",
      },
      kcal: "32",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.0kg - ₹30",
        unit: "kg",
        qty: "1.0",
        rs: "30",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 41,
    name: {
      eng: "Brinjal Ujala",
      tam: "உஜாலா கத்தரிக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/brinjal_ujala.jpeg",
      alt: "Brinjal Ujala",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Brinjal is composed of fat, vitamins, lots of water and minerals that might help with better skin tone and quality of skin, hair, and nail. It may also help with cancer of the skin.2 However, much more extensive research is required in this area to state the above-mentioned potential uses as facts.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "25",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹60",
        unit: "gm",
        qty: "250",
        rs: "60",
      },
      {
        text: "500gm - ₹90",
        unit: "gm",
        qty: "500",
        rs: "90",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 42,
    name: {
      eng: "Brinjal White",
      tam: "வெள்ளை கத்தரிக்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/brinjal_white.png",
      alt: "Brinjal White",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Brinjal is composed of fat, vitamins, lots of water and minerals that might help with better skin tone and quality of skin, hair, and nail. It may also help with cancer of the skin.2 However, much more extensive research is required in this area to state the above-mentioned potential uses as facts.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "25",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹40",
        unit: "gm",
        qty: "250",
        rs: "40",
      },
      {
        text: "500gm - ₹60",
        unit: "gm",
        qty: "500",
        rs: "60",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 43,
    name: {
      eng: "Bullet Chilli",
      tam: "குண்டு மிளகாய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/bullent-chilli.jpeg",
      alt: "Bullet Chilli",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Chili peppers and chili powder are often used to flavor foods, while especially spicy chilis may be reserved for dares. But hidden behind the spicy flavor of this vegetable is an array of health benefits you should know about. Here are just a few of the benefits you may enjoy when eating chilis.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "1",
        unit: "gm",
      },
      kcal: "10",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "100gm - ₹20",
        unit: "gm",
        qty: "100",
        rs: "20",
      },
      {
        text: "200gm - ₹40",
        unit: "gm",
        qty: "200",
        rs: "40",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 44,
    name: {
      eng: "Capsicum Green",
      tam: "பச்சை குடை மிளகாய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/capsium-green.jpeg",
      alt: "Capsicum Green",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Green sweet peppers are a good source of fiber. Adding fiber-rich foods to your diet can promote healthy gut function and protect against conditions that affect the digestive system.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "7",
        unit: "gm",
      },
      kcal: "30",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "250gm - ₹40",
        unit: "gm",
        qty: "250",
        rs: "40",
      },
      {
        text: "500gm - ₹60",
        unit: "gm",
        qty: "500",
        rs: "60",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 45,
    name: {
      eng: "Cluster Beans",
      tam: "கொத்தவரங்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/cluster_beans.png",
      alt: "Cluster Beans",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Iron should be in a soluble and absorbable form to reduce the chances of anemia. Guar is vital source of iron and helps to absorb iron in your body in the most soluble form.",
    nutritions: {
      protein: {
        num: "6",
        unit: "gm",
      },
      carbo: {
        num: "2",
        unit: "gm",
      },
      kcal: "128",
    },
    avail_stock: {
      num: "10",
      unit: "kg",
      into_gram: "10000",
    },
    quantity: [
      {
        text: "250gm - ₹50",
        unit: "gm",
        qty: "250",
        rs: "50",
      },
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 46,
    name: {
      eng: "Coconut Large",
      tam: "தேங்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/cocunt-large.png",
      alt: "Coconut Large",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Though more research is needed, coconut is low in carbs and rich in antioxidants, healthy fats, and fiber, which could potentially support blood sugar control.",
    nutritions: {
      protein: {
        num: "3",
        unit: "gm",
      },
      carbo: {
        num: "15",
        unit: "gm",
      },
      kcal: "354",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "250gm - ₹40",
        unit: "gm",
        qty: "250",
        rs: "40",
      },
      {
        text: "400gm - ₹55",
        unit: "gm",
        qty: "400",
        rs: "55",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 47,
    name: {
      eng: "Cauliflower",
      tam: "காலிஃபிளவர்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/couiflower.png",
      alt: "Cauliflower",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Cauliflower is a cruciferous vegetable that looks like a white version of its cousin, broccoli. Like broccoli, the tightly bunched florets of cauliflower are connected by a thick core, often with a few light leaves surrounding it.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "5",
        unit: "gm",
      },
      kcal: "25",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "1.0kg - ₹70",
        unit: "kg",
        qty: "1.0",
        rs: "70",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 48,
    name: {
      eng: "Cucumber Sambar",
      tam: "சாம்பார் வெள்ளரிக்கா",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/cucmber-sambaar.jpeg",
      alt: "Cucumber Sambar",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Cucumbers are low in calories but high in water and several important vitamins and minerals. Eating cucumbers with the peel provides the maximum amount of nutrients. Cucumbers contain antioxidants, including flavonoids and tannins, which prevent the accumulation of harmful free radicals and may reduce the risk of chronic disease.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "11",
        unit: "gm",
      },
      kcal: "45",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.0kg - ₹80",
        unit: "kg",
        qty: "1.0",
        rs: "80",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 49,
    name: {
      eng: "Potato Agra",
      tam: "ஆக்ரா உருளைக்கிழங்கு",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/potato.jpeg",
      alt: "Potato Agra",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Potatoes are a good source of antioxidants, which may reduce the risk of chronic diseases like eart disease, diabetes and certain cancers. However, more human-based research is required before making any recommendations.",
    nutritions: {
      protein: {
        num: "5",
        unit: "gm",
      },
      carbo: {
        num: "37",
        unit: "gm",
      },
      kcal: "161",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹40",
        unit: "gm",
        qty: "500",
        rs: "40",
      },
      {
        text: "250gm - ₹20",
        unit: "gm",
        qty: "250",
        rs: "20",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 50,
    name: {
      eng: "Ridge Gourd",
      tam: "பீர்க்கங்காய்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/ridge_guard.jpeg",
      alt: "Ridge Gourd",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Ridge gourd is a type of vegetable which is used in Indian cuisine. It has many health benefits and can be consumed in various ways. This vegetable can be consumed in various ways, such as raw, boiled or fried. The ridge gourd has many health benefits including aiding digestion and keeping the skin healthy.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "1",
        unit: "gm",
      },
      kcal: "14",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹50",
        unit: "gm",
        qty: "250",
        rs: "50",
      },
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 51,
    name: {
      eng: "Sweet Corn",
      tam: "இனிப்பு சோளம்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/sweet_corn.jpeg",
      alt: "Sweet Corn",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Corn has a high concentration of vitamin B1. This vitamin is often known as thiamine. Thiamine is required to make acetylcholine in our bodies. Acetylcholine is a neurotransmitter that helps people remember. Thus, having good levels of acetylcholine may improve memory. In addition, it minimises the risk of Alzheimer’s disease. Hence, eating corn may help as a memory booster.",
    nutritions: {
      protein: {
        num: "4",
        unit: "gm",
      },
      carbo: {
        num: "19",
        unit: "gm",
      },
      kcal: "86",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "250gm - ₹20",
        unit: "gm",
        qty: "250",
        rs: "20",
      },
      {
        text: "500gm - ₹40",
        unit: "gm",
        qty: "500",
        rs: "40",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 52,
    name: {
      eng: "Tomato Local",
      tam: "நாட்டு தக்காளி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/tomato_local.png",
      alt: "Tomato Local",
    },
    category: {
      id: "03",
      name: "Fresh Veggies",
    },
    description:
      "Fresh tomatoes are low in carbs. The carb content consists mainly of simple sugars and insoluble fibers. These fruits are mostly made up of water. Tomatoes are a good source of several vitamins and minerals, such as vitamin C, potassium, vitamin K, and folate.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "4",
        unit: "gm",
      },
      kcal: "18",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "1.2gm - ₹60",
        unit: "gm",
        qty: "1.2",
        rs: "60",
        into_gram: "1200",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 53,
    name: {
      eng: "Agathi Keerai",
      tam: "அகத்தி கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/agathi-keerai.png",
      alt: "Agathi Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Agathi Keerai is classified under the green leafy vegetables category. It has cooling properties and is rich in fibre, so it helps in digestion. It helps to cure acidity related issues. Agathi leaves strengthen the nerves of eyes. And can also cure night blindness.",
    nutritions: {
      protein: {
        num: "1",
        unit: "gm",
      },
      carbo: {
        num: "23",
        unit: "gm",
      },
      kcal: "90",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹15",
        unit: "nos",
        qty: "1",
        rs: "15",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 54,
    name: {
      eng: "Arai Keerai",
      tam: "அரைக்கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/ARAIKEERAI.png",
      alt: "Arai Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "It is meant to increase red blood cells. It increases immunity towards cold and cough. It is rich in Iron. It is a blood purifier. It can be taken by pregnant women for the sake of babies and their own Hb count.",
    nutritions: {
      protein: {
        num: "4",
        unit: "gm",
      },
      carbo: {
        num: "2",
        unit: "gm",
      },
      kcal: "36",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹25",
        unit: "nos",
        qty: "1",
        rs: "25",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 55,
    name: {
      eng: "Banana Leaf",
      tam: "வாழை இலை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/BANANALEAF.png",
      alt: "Banana Leaf",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Banana leaves extract can cure dandruffs, maintain healthy scalp, and keep the natural color of the hair, thanks to its high allantoin and astringent effect. If we often have itchy and sore scalp, this banana leaves hair mask is proven to be very effective.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹12",
        unit: "nos",
        qty: "1",
        rs: "12",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 56,
    name: {
      eng: "Betel",
      tam: "வெற்றிலை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/bettel.jpeg",
      alt: "Betel",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Betel leaves are rich in antioxidants that combat oxidative stress in diabetic patients. This reduces blood glucose levels and thus acts as an anti-diabetic agent.",
    nutritions: {
      protein: {
        num: "3",
        unit: "gm",
      },
      carbo: {
        num: "6",
        unit: "gm",
      },
      kcal: "61",
    },
    avail_stock: {
      num: "50",
      unit: "nos",
    },
    quantity: [
      {
        text: "10nos - ₹15",
        unit: "nos",
        qty: "10",
        rs: "15",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 57,
    name: {
      eng: "Curry Leaves",
      tam: "கறிவேப்பிலை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/CURRYLEAVES.png",
      alt: "Curry Leaves",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "curry leaves are rich in antioxidant properties, which helps in preventing the oxidation of cholesterol that is responsible for the formation of bad cholesterol. It also helps in increasing the good cholesterol and protects the body from cardiovascular diseases.",
    nutritions: {
      protein: {
        num: "8",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "97",
    },
    avail_stock: {
      num: "15",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹10",
        unit: "nos",
        qty: "1",
        rs: "10",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 58,
    name: {
      eng: "Karisalanganni Keerai",
      tam: "கரிசலாங்கண்ணி கீரை",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/Karisalanganni-Keerai.png",
      alt: "Karisalanganni Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Due to its powerful antibacterial and antimicrobial properties, Karisalankanni is also beneficial for scalp problems such as dandruff, itching, flaking, etc. It also helps in maintaining the natural colour of hair and prevents premature greying.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹20",
        unit: "nos",
        qty: "1",
        rs: "20",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 59,
    name: {
      eng: "Manathakkali Keerai",
      tam: "மனத்தக்காளி கீரை",
    },
    image: {
      source:
        "https://freeimghost.net/images/2023/03/01/MANATHAKALLIKEERAI.png",
      alt: "Manathakkali Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "One of the significant health benefits of manathakkali keerai or black nightshade is curing ulcers. The anti-ulcer properties in the plant can help anyone with an ulcer. Boil manathakkali in coconut milk and add a pinch of salt and have this soup. You will quickly see the difference in treating ulcer pain. Scientific research showed that manathakkali prevents acid secretion that causes ulcers.",
    nutritions: {
      protein: {
        num: "6",
        unit: "gm",
      },
      carbo: {
        num: "9",
        unit: "gm",
      },
      kcal: "68",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹15",
        unit: "nos",
        qty: "1",
        rs: "15",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 60,
    name: {
      eng: "Mint Leaves",
      tam: "புதினா",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/MINTLEAVES.png",
      alt: "Mint Leaves",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Bad Smells of breath. Fresh mint flavor makes it ideal to relieve bad Smells of breath odors. Mint tea made from fresh mint leaves or dried effectively get rid of bacteria that cause bad smells of breath. Mint will also keep the mouth moist by stimulating saliva production.",
    nutritions: {
      protein: {
        num: "4",
        unit: "gm",
      },
      carbo: {
        num: "9",
        unit: "gm",
      },
      kcal: "68",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹9",
        unit: "nos",
        qty: "1",
        rs: "9",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 61,
    name: {
      eng: "Mudakathan Keerai",
      tam: "முடக்கத்தான் கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/mudakathan.png",
      alt: "Mudakathan Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "The extract of the mudakathan keerai has the antidiarrhoeal property; due to this property, it is traditionally used for particular places in India. Some researches proved that this mudakathan keerai has flavonoids, tannins, sterols, alkaloids, saponins, and triterpenes.",
    nutritions: {
      protein: {
        num: "5",
        unit: "gm",
      },
      carbo: {
        num: "9",
        unit: "gm",
      },
      kcal: "61",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹13",
        unit: "nos",
        qty: "1",
        rs: "13",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 62,
    name: {
      eng: "Mulai Keerai",
      tam: "முளைக்கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/MULAIKEERAI.png",
      alt: "Mulai Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Mulai keerai is a rich source of dietary fiber, calcium, protein, iron, vitamin A, B2, B6, C and some other essential nutrients. The leaves of mulai keerai have medicinal purposes. People suffering with kidney problems, gallstones are advised not to consume mulai keerai.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "42",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹25",
        unit: "nos",
        qty: "1",
        rs: "25",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 63,
    name: {
      eng: "Moringa Leaves",
      tam: "முருங்கைக்கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/MURUNGAIKEERAI.png",
      alt: "Moringa Leaves",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "When eating a murungai keerai soup when the flu, this will make the body become warmer. When the body becomes warmer, the flu can subside and heal slowly. But remember, the murungai keerai soup consumed when the flu is this soup that is still warm.",
    nutritions: {
      protein: {
        num: "3",
        unit: "gm",
      },
      carbo: {
        num: "4",
        unit: "gm",
      },
      kcal: "27",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹57",
        unit: "nos",
        qty: "1",
        rs: "57",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 64,
    name: {
      eng: "Paruppu Keerai",
      tam: "பருப்பு கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/paruup-keerai.png",
      alt: "Paruppu Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Paruppu keerai is a great appetizer and it invokes hunger in kids. It is very much healthy and helps in reducing bad cholesterol in the body. Paruppu keerai also helps in purifying the blood and detoxifies the impurities in the liver. It is always good to have parupu keerai with dhal and garlic, because it gives a great taste and lots of health benefits to the body.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "3",
        unit: "gm",
      },
      kcal: "29",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹13",
        unit: "nos",
        qty: "1",
        rs: "13",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 65,
    name: {
      eng: "Pasalai Keerai",
      tam: "பசலைக்கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/PASALAIKEERAI.png",
      alt: "Pasalai Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Consuming the stem and leaf in the regular diet helps overcome the common cold, cough, infections caused due to cold and anemia.",
    nutritions: {
      protein: {
        num: "6",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "41",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹19",
        unit: "nos",
        qty: "1",
        rs: "19",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 66,
    name: {
      eng: "Pirandai",
      tam: "பிரண்டை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/pirandai.png",
      alt: "Pirandai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Pirandai has wonderful antioxidant properties and great helps reduce free radicals that are responsible for the premature ageing of our skin and body. The antioxidant property is attributed mainly due to the presence of high amount of beta carotene in pirandai plant.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "pkt",
    },
    quantity: [
      {
        text: "1pkt - ₹25",
        unit: "pkt",
        qty: "1",
        rs: "25",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 67,
    name: {
      eng: "Pulicha Keerai",
      tam: "புளிச்ச கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/pulichai-keerai.png",
      alt: "Pulicha Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Several studies have proven that gongura has lipid lowering properties which could greatly prevent cardio vascular diseases. The water extract of the plant reduces LDL, triglyceride and total cholesterol levels. This is due to it’s wonderful antioxidant properties. Roselle tea can also be used for reducing cholesterol levels.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹15",
        unit: "nos",
        qty: "1",
        rs: "15",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 68,
    name: {
      eng: "Siru Keerai",
      tam: "சிறுக்கீரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/SIRUKEERAI.png",
      alt: "Siru Keerai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "It grows faster then other greens and available in all the seasons. It has iron and calcium in abundant quantity. It increases memory power if it’s taken in a daily diet. The other names for siru keerai are silly, sagini and mega nadam.It excretes the unwanted materials and protects the body form dangerous diseases.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹25",
        unit: "nos",
        qty: "1",
        rs: "25",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 69,
    name: {
      eng: "Thoothuvalai",
      tam: "தூதுவளை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/thoothuvalai.png",
      alt: "Thoothuvalai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Thoothuvalai is widely used in the treatment of tuberculosis, bronchial asthma, difficulty in breathing, sinus, chest congestion and other respiratory problems. Thoothuvalai with its anti-inflammatory properties and anti-microbial activity helps asthma patients with a fresh breath of life.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹15",
        unit: "nos",
        qty: "1",
        rs: "15",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 70,
    name: {
      eng: "Vallarai",
      tam: "வல்லாரை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/vallarai.png",
      alt: "Vallarai",
    },
    category: {
      id: "05",
      name: "Leafy Green",
    },
    description:
      "Recent studies have indicated that Vallarai is neuro-protective in nature. Vallarai protects brain cells and rejuvenates them. It helps to improve memory and concentration span.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "nos",
    },
    quantity: [
      {
        text: "1nos - ₹14",
        unit: "nos",
        qty: "1",
        rs: "14",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 71,
    name: {
      eng: "Cheppan Kilangu",
      tam: "சேபங்கிழங்கு",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/chippan-kelangu.jpeg",
      alt: "Cheppan Kilangu",
    },
    category: {
      id: "06",
      name: "Tubers",
    },
    description:
      "Taro root contains fiber and resistant starch, which both slow digestion and reduce blood sugar spikes after meals.Taro root is a good source of fiber and many vitamins and minerals that the standard American diet frequently lacks.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "15",
      unit: "kg",
      into_gram: "15000",
    },
    quantity: [
      {
        text: "1kg - ₹37",
        unit: "kg",
        qty: "1",
        rs: "37",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 73,
    name: {
      eng: "Karunai Kilangu",
      tam: "கருணை கிழங்கு",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/karunai-kilangu.jpeg",
      alt: "Karunai Kilangu",
    },
    category: {
      id: "06",
      name: "Tubers",
    },
    description: "Karunai Kilangu Description.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹33",
        unit: "gm",
        qty: "500",
        rs: "33",
      },
      {
        text: "1kg - ₹70",
        unit: "kg",
        qty: "1",
        rs: "70",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 74,
    name: {
      eng: "Pidi Karunai",
      tam: "பிடி கருணை",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/pidikarunai.png",
      alt: "Pidi Karunai",
    },
    category: {
      id: "06",
      name: "Tubers",
    },
    description: "Pidi Karunai Description.",
    nutritions: {
      protein: {
        num: "0",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "00",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹26",
        unit: "gm",
        qty: "500",
        rs: "26",
      },
      {
        text: "1kg - ₹53",
        unit: "kg",
        qty: "1",
        rs: "53",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 75,
    name: {
      eng: "Sweet Potato",
      tam: "சர்க்கரைவள்ளி",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/sweet-potato.jpeg",
      alt: "Sweet Potato",
    },
    category: {
      id: "06",
      name: "Tubers",
    },
    description:
      "Sweet potatoes are mainly composed of carbs. Most of the carbs come from starch, followed by fiber. This root vegetable is also relatively low in protein but still an important protein source in many developing countries.",
    nutritions: {
      protein: {
        num: "2",
        unit: "gm",
      },
      carbo: {
        num: "0",
        unit: "gm",
      },
      kcal: "87",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "250gm - ₹40",
        unit: "gm",
        qty: "250",
        rs: "40",
      },
      {
        text: "500gm - ₹70",
        unit: "gm",
        qty: "500",
        rs: "70",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
  {
    id: 76,
    name: {
      eng: "Apple Indian",
      tam: "ஆப்பிள் இந்தியன்",
    },
    image: {
      source: "https://freeimghost.net/images/2023/03/01/apple_india.png",
      alt: "Apple Indian",
    },
    category: {
      id: "04",
      name: "Fresh Fruits",
    },
    description:
      "This mighty fruit is known to maintain oral health. Thanks to malic acid that keeps your teeth white and shiny. It is widely used by companies to prepare commercial whitening toothpaste. It is worth mentioning that chewing apple stimulates saliva production that keeps risks of tooth decay at bay.",
    nutritions: {
      protein: {
        num: "26",
        unit: "gm",
      },
      carbo: {
        num: "81",
        unit: "gm",
      },
      kcal: "72",
    },
    avail_stock: {
      num: "20",
      unit: "kg",
      into_gram: "20000",
    },
    quantity: [
      {
        text: "500gm - ₹80",
        unit: "gm",
        qty: "500",
        rs: "80",
      },
      {
        text: "1kg - ₹156",
        unit: "kg",
        qty: "1",
        rs: "156",
        into_gram: "1000",
      },
    ],
    status: true,
    farmer: {
      id: "01",
      name: "Nammalvar G",
      image: {
        source: "https://freeimghost.net/images/2023/03/01/nammalvar.jpeg",
        alt: "Nammalvar G",
      },
    },
  },
];

// setting the created products on the localstorage if it is not set

const get_products = JSON.parse(localStorage.getItem("product_list"));

function append_products() {
  if (get_products) {
    return false;
  }
  localStorage.setItem("product_list", JSON.stringify(created_products));
  return true;
}

append_products();
