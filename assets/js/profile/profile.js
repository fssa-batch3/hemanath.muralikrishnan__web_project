...
let user_profile = user_records.find(function find_user(obj) {
...
profile_img.setAttribute("src", "https://ui-avatars.com/api/?name=" + user_profile.firstname + " " + user_profile.lastname + "&background=0D8ABC&color=fff");
profile_img.setAttribute("alt", "user image of " + user_profile.firstname + " " + user_profile.lastname);
...
function to_save_address(address_value, district_value, state_value, pincode_value) {
...
function load_address() {
    ...
    let user_add_arr = user_profile.address;

    if (user_add_arr != null) {

        if ((user_add_arr.length) > 0) {

            let address_array = user_profile["address"];
...
function show_address(address_array = []) {
    ...
    address_array.forEach((item, index) => {
        ...
        address_p_edit.setAttribute("onclick", `update_address(${JSON.stringify(item)})`);
