...
for(k=0; k<product_details.length; k++){
    if(product_id == product_details[k]["product"].id){
...
indv_product_image.setAttribute("src", product_details[k]["product"]["image"]["source"]);
indv_product_image.setAttribute("alt", "image of " + product_details[k]["product"]["image"]["alt"]);
...
product_name_english_p.innerText = product_details[k]["product"]["name"]["english"];
...
product_name_tamil_p.innerText = product_details[k]["product"]["name"]["tamil"];
...
pro_circle_p.innerText = product_details[k]["product"]["healthy"]["protein"] + "g";
...
carbo_circle_p.innerText = product_details[k]["product"]["healthy"]["carbo"] + "g";
...
kcal_circle_p.innerText = product_details[k]["product"]["healthy"]["kcal"];
...
indv_category_title_p.innerText = product_details[k]["product"]["category"]["title"];
...
indv_farmer_img.setAttribute("alt", "image of farmer " + product_details[0]["farmer"]["image"]["alt"]);
...
indv_product_name_english_p.innerText = product_details[k]["product"]["name"]["english"];
...
indv_product_name_tamil_p.innerText = product_details[k]["product"]["name"]["tamil"];
...
let dropdown_values = product_details[k]["product"]["select_dropdown"];

dropdown_values.forEach((item) => {
    const option = document.createElement("option");
    option.value = item.value_source;
...
select_tag.addEventListener("change", function () {
    ...
    indv_amount_one_p.innerText = "₹ " + product_details[k]["product"]["unit_price"] * selectedValue;
});
...
indv_amount_one_p.innerText = "₹ " + product_details[k]["product"]["unit_price"];
...
desc_content_p.innerText = product_details[k]["product"]["description"];
...
for(let j=0; j<product_details.length;j++){
    ...
    if(product_details[j]["product"]["category"]["id"] == product_cat){

        let product = product_details[j]["product"];
    ...
    product_image_src.setAttribute("src", product_details[j]["product"]["image"]["source"]);
    product_image_src.setAttribute("alt", product_details[j]["product"]["image"]["alt"]);
    ...
    product_cat_title.innerHTML = product_details[j]["product"]["category"]["title"];
    ...
    product_eng_name_p.innerText = product_details[j]["product"]["name"]["english"];
    product_name_div.append(product_eng_name_p);

    // product tam name
    product_tam_name_p = document.createElement("p");
    product_tam_name_p.innerText = product_details[j]["product"]["name"]["tamil"];
    ...
    rating_num.innerText = product_details[j]["product"]["rating"];
